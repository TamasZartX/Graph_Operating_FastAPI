...
from schemas import GraphCreate, GraphCreateResponse, AdjacencyListResponse
...
@router.get("/{graph_id}/adjacency_list", response_model=AdjacencyListResponse)
def get_adj_list(graph_id: int):
    adj_list = read_graph(graph_id, "adjacency_list")
    return {"adjacency_list": adj_list}


@router.get("/{graph_id}/reverse_adjacency_list", response_model=AdjacencyListResponse)
